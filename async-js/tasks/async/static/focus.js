...
function sendRequest(url) {
  return fetch(url).then((response) => {
    if (response.ok) return response.json();
      ...
      alert(
        `Response code: ${response.status}. Response status: ${response.statusText}`
      );
  });
}
...
async function run() {
  try {
    ...
    const requisitesRequest = sendRequest(`${API.orgReqs}?ogrn=${ogrns}`);
    const analyticsRequest = sendRequest(`${API.analytics}?ogrn=${ogrns}`);
    const buhRequest = sendRequest(`${API.buhForms}?ogrn=${ogrns}`);

    const [requisitesResponse, analyticsResponse, buhresponse] = await Promise.all([requisitesRequest, analyticsRequest, buhRequest]);

    const orgsMap = reqsToMap(requisitesResponse);
    addInOrgsMap(orgsMap, analyticsResponse, "analytics");
    addInOrgsMap(orgsMap, buhresponse, "buhForms");
